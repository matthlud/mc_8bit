...
% Include the Microarchitecture chapter
\input{microarch.tex}

The following SystemVerilog-style sequence demonstrates how to initialize the memory for a test program. The writes are synchronous and should be driven away from the active clock edge.
\\
\textbf{Program Logic:} Load immediate 5, add 3 from memory, store the result, load another value, subtract, store, then halt.
...
// Program: LDI 5; ADD [1]; STA [2]; LDA [3]; SUB [2]; STA [4]; HLT
